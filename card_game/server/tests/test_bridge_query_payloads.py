...
def test_parse_frontend_input_result_accepts_ordered_selections_for_none() -> None:
    card_a = _FakeCard('card-a')
    ...
    parsed = bridge._parse_frontend_input_result(event, {'ordered_selections': ['none']})

    assert isinstance(parsed, dict)
    assert parsed.get('input_result') == [None]


def test_parse_frontend_input_result_rejects_single_string_ordered_selections() -> None:
    card_a = _FakeCard('card-a')
    bridge = _make_bridge_with_cards(card_a)
...
def test_parse_frontend_input_result_pads_missing_allows_none_card_selections() -> None:
    ...
    parsed = bridge._parse_frontend_input_result(event, {'ordered_selections': ['card-a']})
...
def test_parse_frontend_input_result_for_multi_d6_values() -> None:
    ...
    parsed = bridge._parse_frontend_input_result(event, {'result_values': [6, 2]})
